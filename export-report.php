...
function generateReportHTML($reports, $month, $year, $department) {
    ...
    <table border='1' cellpadding='5'>
        <tr>
            ...
            <td>" . formatCurrency($monthlyStats['total_amount'] ?? 0) . "</td>
        </tr>
        <tr>
            <td><strong>Yearly Expenses:</strong></td>
            <td>" . formatCurrency($yearlyStats['total_amount'] ?? 0) . "</td>
        </tr>
        <tr>
            <td><strong>Total Retired:</strong></td>
            <td>" . formatCurrency($retirementStats['total_retired'] ?? 0) . "</td>
        </tr>
        <tr>
            <td><strong>Amount Returned:</strong></td>
            <td>" . formatCurrency($retirementStats['total_returned'] ?? 0) . "</td>
        </tr>
    </table>
...
function generateExcelReport($reports, $month, $year, $department) {
    ...
    $output .= "Monthly Expenses\t" . ($monthlyStats['total_amount'] ?? 0) . "\n";
    $output .= "Yearly Expenses\t" . ($yearlyStats['total_amount'] ?? 0) . "\n";
    $output .= "Total Retired\t" . ($retirementStats['total_retired'] ?? 0) . "\n";
    $output .= "Amount Returned\t" . ($retirementStats['total_returned'] ?? 0) . "\n\n";
